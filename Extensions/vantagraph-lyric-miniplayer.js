// Vantagraph Lyric Miniplayer - PiP window: seekbar, word-sync karaoke, translations, rAF loop
//
...
//
// Vantagraph improvements: live --spice-* theming, rAF loop, VantagraphData integration.
// v2: Spotify-like layout, gear icon, repeat, grouped settings, VG icons, faded lyrics, theme refresh.
...
(async function VantagraphLyricMiniplayer() {
  ...
  let useVgIcons       = ls('usevgicons')  !== null ? ls('usevgicons')  === 'true' : true;
  ...
  // 1. VG icons
  const vgIconData = window.__vantagraphIconData || {};
  function makeIconHtml(vgKey, fallbackPath, size) {
    size = size || 16;
    const data = useVgIcons && vgIconData[vgKey];
    if (data) return `<div style="width:${size}px;height:${size}px;background:currentColor;flex-shrink:0;-webkit-mask-image:url('${data}');mask-image:url('${data}');-webkit-mask-size:contain;mask-size:contain;-webkit-mask-repeat:no-repeat;mask-repeat:no-repeat;-webkit-mask-position:center;mask-position:center;"></div>`;
    return `<svg viewBox="0 0 16 16" width="${size}" height="${size}" fill="currentColor" style="flex-shrink:0;">${fallbackPath}</svg>`;
  }
  ...
  function makeVolIconHtml(vol, muted) {
    ...
    if (useVgIcons && vgIconData['bottombar-VolumeOn.svg']) return makeIconHtml('bottombar-VolumeOn.svg', P.volHigh, 13);
  ...
  function setupSettingsWindow(win) {
    ...
    <div class="s-content">
      ...
      <div class="s-group">
        ...
        ${tItem('vgicons', 'Vantagraph Icons', useVgIcons, 'Reopen to apply')}
      </div>
      <div class="s-group">
      ...
      <div class="s-group">
        ...
        <div class="s-item" style="cursor:default">
          <span class="s-label">Vantagraph Settings</span>
    ...
    win.__vg = {
      toggle(key) {
        ...
        const map = {
          ...
          vgicons:  () => { useVgIcons = !useVgIcons;                                                                                                                              lss('usevgicons',  useVgIcons);       return useVgIcons; },
  ...
  // vantagraph-taskbarplayer.js expand button signals us to open the full PiP
