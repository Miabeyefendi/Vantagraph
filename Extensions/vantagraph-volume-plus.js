// Vantagraph Volume+ - scroll wheel + middle-click mute + preset overlay + wide bar + tooltip
//
...
(async function VantagraphVolumePlus() {
  ...
  if (!document.getElementById("vg-volume-plus-css")) {
    ...
    s.textContent = `
      .vg-vol-preset-trigger { background:none; border:none; color:var(--spice-subtext); cursor:pointer; padding:4px; margin-left:2px; display:flex; align-items:center; justify-content:center; border-radius:50%; transition:color .15s; }
  ...
  presetBtn.innerHTML = `<svg viewBox="0 0 16 16"><text x="50%" y="55%" dominant-baseline="middle" text-anchor="middle" font-size="11" font-weight="700" fill="currentColor">%</text></svg>`;
  try {
    const iconData = window.__vantagraphIconData;
    if (iconData && iconData["bottombar-percentage.svg"]) {
      const iconDiv = document.createElement("div");
      iconDiv.className = "vg-vol-preset-icon";
      iconDiv.style.setProperty("-webkit-mask-image", `url("${iconData["bottombar-percentage.svg"]}")`, "important");
      iconDiv.style.setProperty("mask-image",         `url("${iconData["bottombar-percentage.svg"]}")`, "important");
      presetBtn.innerHTML = "";
      presetBtn.appendChild(iconDiv);
    }
  } catch(e) {}
