...
(async function VantagraphTaskbarPlayer() {
  ...
  // ── VG ICONS ───────────────────────────────────────────────────────────────
  const vgIcons = window.__vantagraphIconData || {};

  function icon(vgKey, svgPath, size = 16) {
    const d = vgIcons[vgKey];
    if (d) return `<div style="width:${size}px;height:${size}px;background:currentColor;flex-shrink:0;-webkit-mask-image:url('${d}');mask-image:url('${d}');-webkit-mask-size:contain;mask-size:contain;-webkit-mask-repeat:no-repeat;mask-repeat:no-repeat;-webkit-mask-position:center;mask-position:center;"></div>`;
  ...
  // vantagraph-lyric-miniplayer.js miniBtn dispatches this
